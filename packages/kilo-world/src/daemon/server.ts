...
export namespace DaemonServer {
    ...
    try {
      ...
      if (Number.isFinite(pid)) {
        try {
          ...
          if (cause instanceof Error && cause.message.includes("already starting")) throw cause
        }
      }
      remove(file)
      writeFileSync(file, String(process.pid), { flag: "wx", mode: 0o600 })
  ...
  async function shutdown(sessionID: string, silent: boolean): Promise<void> {
    ...
    await installedShutdown?.().catch((err) => log(`browser shutdown failed: ${String(err)}`, silent))
  ...
  export async function start(opts: Options): Promise<Server> {
    ...
    const idleTimeoutMs = Math.max(0, Math.floor(rawIdle))
