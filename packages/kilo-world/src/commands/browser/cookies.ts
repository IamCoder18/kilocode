...
export namespace Cookies {
  ...
  export async function get(input: { session?: string; domain?: string }): Promise<{ cookies: CookieEntry[]; count: number }> {
    ...
    const filtered = input.domain ? all.filter((c: Cookie) => matchesDomain(c, input.domain!)) : all
  ...
  export async function set(input: {
    ...
    const name = input.session ?? "default"
    const live = await Runner.attach(name)
    const cookie: AddCookieInput = input.url
      ? { name: input.name, value: input.value, url: input.url }
      : { name: input.name, value: input.value, domain: input.domain, path: input.path ?? "/" }
    await live.context.addCookies([cookie])
    const stored: Cookie = {
      name: cookie.name,
      value: cookie.value,
      domain: "domain" in cookie ? cookie.domain! : input.domain,
