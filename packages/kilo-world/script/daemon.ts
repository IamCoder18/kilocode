...
export namespace WorldDaemon {
  export const filename = "world-daemon.cjs"
  ...
  export async function copy(bundle: Bundle, dir: string): Promise<string> {
    ...
    for (const file of bundle.files) {
      ...
      await Bun.write(path.join(dir, path.basename(file.path)), file)
    }
    return path.join(dir, filename)
  }

  export async function smoke(file: string): Promise<void> {
    const node = Bun.which("node")
    if (!node) throw new Error("Node is required to smoke-test the kilo-world daemon")
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kilo-world-build-"))
    const session = `smoke-${process.pid}`
    const handshake = path.join(root, `daemon-${session}.json`)
    const child = spawn(node, [file, `--session=${session}`, "--idle=0"], {
      env: {
        ...process.env,
